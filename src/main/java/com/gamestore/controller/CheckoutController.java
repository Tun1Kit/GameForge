    ...
    public String processCheckout(
        ...
        for (CartItem item : cartItems) {
            ...
            if (game.getPublisher() != null && game.getPublisher().getUser() != null) {
                if (game.getPublisher().getUser().getId().equals(managedUser.getId())) {
                    return buildCheckoutError(model, cartItemDAO, managedUser.getId(),
                        "Bạn không thể mua game '" + game.getTitle() + "' vì bạn là nhà phát hành của game này.");
                }
            }
            if (libraryItemDAO.existsActiveByUserAndGame(managedUser.getId(), game.getId())
                ...
                return buildCheckoutError(model, cartItemDAO, managedUser.getId(),
                    "Bạn đã sở hữu hoặc đang chờ cấp key cho game '" + game.getTitle() + "'. Vui lòng xóa khỏi giỏ hàng.");
            }
        }
