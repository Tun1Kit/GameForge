...
<body class="position-relative" style="min-height:100vh;overflow-x:hidden;">
    ...
    <div class="container-xl py-3">
      <div class="d-flex align-items-center justify-content-between gap-3">
        ...
        <div class="d-flex align-items-center gap-2">
          ...
          <a href="${pageContext.request.contextPath}/admin/settings" class="btn btn-sm gf-border-2 gf-press text-white fw-bold rounded-3 d-flex align-items-center gap-2 py-2 px-3" style="background:var(--gf-green);border-color:#000;">
  ...
  <main class="container-xl py-5">
    <div class="mb-4">
      <div class="d-inline-flex align-items-center gap-2 gf-border-2 rounded-pill px-4 py-2 fw-bold text-black mb-3" style="background:var(--gf-lavender);box-shadow:3px 3px 0 #000;">
      ...
      <p class="fs-5 fw-semibold text-secondary gf-muted" style="max-width:500px;">Cấu hình các thông số vận hành nền tảng GameForge.</p>
    </div>
    ...
    <c:if test="${not empty sessionScope.settingSuccess}">
      <div class="alert gf-border-2 gf-shadow-sm mb-4 fw-bold d-flex align-items-center gap-2" style="background:#94FFB4;border:3px solid #000;border-radius:12px;" role="alert">
        <i data-lucide="check-circle" width="18" height="18"></i> ${fn:escapeXml(sessionScope.settingSuccess)}
    ...
    <c:if test="${not empty sessionScope.settingError}">
      <div class="alert gf-border-2 gf-shadow-sm mb-4 fw-bold d-flex align-items-center gap-2" style="background:var(--gf-pink);border:3px solid #000;border-radius:12px;" role="alert">
        <i data-lucide="alert-circle" width="18" height="18"></i> ${fn:escapeXml(sessionScope.settingError)}
    ...
    <div class="bg-white gf-border rounded-4 gf-shadow overflow-hidden mb-4">
      <div class="p-3 fw-black d-flex align-items-center gap-2" style="background:var(--gf-yellow);border-bottom:3px solid #000;">
        <i data-lucide="percent" width="18" height="18"></i> Phí nền tảng (Platform Commission)
      </div>
      <div class="p-4">
        <form action="${pageContext.request.contextPath}/admin/settings" method="post" id="commissionForm">
          <input type="hidden" name="_csrf" value="${_csrf.token}" />
          <input type="hidden" name="action" value="updateCommission" />
          <div class="row g-4 align-items-end">
            <div class="col-md-6">
              <label class="form-label fw-bold small text-secondary">Tỷ lệ phí hoa hồng nền tảng (%)</label>
              <div class="position-relative">
                <input type="number" name="commissionRate" id="commissionRate" value="${commissionRate}" min="0" max="100" step="0.1" class="form-control gf-input ps-5" placeholder="VD: 15.0" required>
                <span class="position-absolute fw-black" style="top:50%;left:14px;transform:translateY(-50%);font-size:18px;">%</span>
              </div>
              <div class="form-text small">Phần trăm doanh thu game nền tảng giữ lại từ mỗi giao dịch.</div>
            </div>
            <div class="col-md-3">
              <div class="p-3 gf-border-2 rounded-3" style="background:var(--gf-lavender);border:2px solid #000;">
                <div class="small fw-bold text-secondary">Hiện tại</div>
                <div class="fs-4 fw-black">${commissionRate}%</div>
              </div>
            </div>
            <div class="col-md-3">
              <button type="submit" class="btn w-100 gf-border gf-shadow-sm gf-press fw-bold d-flex align-items-center justify-content-center gap-2" style="background:var(--gf-green);border-radius:10px;padding:10px 20px;">
                <i data-lucide="save" width="16" height="16"></i> Lưu phí hoa hồng
              </button>
            </div>
          </div>
        </form>
    ...
    <div class="bg-white gf-border rounded-4 gf-shadow overflow-hidden">
      <div class="p-3 fw-black d-flex align-items-center gap-2" style="background:var(--gf-lavender);border-bottom:3px solid #000;">
        <i data-lucide="info" width="18" height="18"></i> Thông tin hệ thống
      </div>
      <div class="p-4">
        <div class="row g-4">
