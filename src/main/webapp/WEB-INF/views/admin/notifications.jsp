...
<head>
  ...
  <title>GameForge - Admin Notifications</title>
    ...
    <div class="container-xl py-2">
      <div class="d-flex align-items-center justify-content-between gap-3">
        ...
        <div class="d-flex align-items-center gap-2 flex-grow-1 justify-content-end flex-wrap">
          ...
          <!-- Notification Bell -->
          <a href="${pageContext.request.contextPath}/admin/notifications" class="btn btn-sm gf-border-2 gf-press text-white rounded-3 d-flex align-items-center justify-content-center p-0 position-relative" style="width:38px; height:38px; border-color:#000; background:var(--gf-green); flex-shrink:0; color:#fff !important;">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-5">
      <div>
        ...
        <h1 class="fw-black fw-bold mb-2" style="font-size:clamp(2rem,5vw,3rem);line-height:1.1;letter-spacing:-0.03em;">
          Thông báo <span style="color:var(--gf-green);text-shadow:2px 2px 0 #000;">Admin</span>
        </h1>
        <p class="fs-5 fw-semibold text-secondary gf-muted mb-0">Theo dõi các yêu cầu phê duyệt game, KYC, và payout mới nhất.</p>
      </div>

      <c:if test="${not empty notifications}">
        <form action="${pageContext.request.contextPath}/admin/notifications/mark-read" method="POST" class="flex-shrink-0">
    ...
    <c:choose>
      <c:when test="${empty notifications}">
        <div class="bg-white gf-border rounded-4 gf-shadow p-5 text-center my-4">
          ...
          <p class="text-secondary fw-semibold gf-muted mb-0">Tuyệt vời! Bạn đã xem hết tất cả thông báo hệ thống.</p>
        </div>
      </c:when>
      <c:otherwise>
              ...
              <div class="d-flex justify-content-between align-items-start gap-3">
                <div class="d-flex align-items-start gap-3">
                  <div class="gf-border-2 rounded-3 d-flex align-items-center justify-content-center flex-shrink-0" 
                       style="width: 44px; height: 44px; background: ${notif.type == 'KYC' ? 'var(--gf-yellow)' : notif.type == 'PAYOUT' ? 'var(--gf-pink)' : notif.type == 'GAME_APPROVAL' ? 'var(--gf-blue)' : 'var(--gf-lavender)'};">
                    <c:choose>
                      <c:when test="${notif.type == 'KYC'}">
                        <i data-lucide="id-card" width="20" height="20"></i>
                      </c:when>
                ...
                <c:if test="${not empty notif.targetUrl}">
                  <a href="${pageContext.request.contextPath}${notif.targetUrl}" class="btn btn-sm gf-border-2 gf-press bg-white text-dark fw-bold rounded-3 px-3 py-2 d-flex align-items-center gap-2 flex-shrink-0 align-self-center">
                    <i data-lucide="external-link" width="14" height="14"></i> Xử lý ngay
                  </a>
                </c:if>
              </div>
            </div>
