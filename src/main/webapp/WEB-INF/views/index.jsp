...
<%
  ...
  // Dùng session chứ không phải pageContext — vì currentUser được set vào HttpSession từ controller
    ...
    <div class="container-xl py-3">
      <div class="d-flex align-items-center justify-content-between gap-3">
        ...
        <div class="d-flex align-items-center gap-2 gap-sm-3">
          ...
          <button id="cartTopBtn" class="gf-icon-btn gf-press" style="background:var(--gf-blue)" type="button" title="Giỏ hàng">
                ...
                <ul class="dropdown-menu dropdown-menu-end gf-border-2 gf-shadow-sm p-2" style="border-radius: 12px; min-width: 210px;">
                  ...
                  <li>
                    <div class="px-3 py-1.5 d-flex align-items-center justify-content-between gap-2 bg-light rounded-3 gf-border">
                      ...
                      <span class="fw-black text-success" id="headerWalletBalance" style="font-size: 14px; font-weight: 900 !important;">
                        <c:choose>
...
<a class="dropdown-item d-flex align-items-center gap-2 fw-bold py-2 text-danger"
      ...
      <section id="store" class="gf-hero">
        <div class="container-xl">
          <div class="row align-items-start g-5">
            ...
            <div class="col-lg-6">
              <div class="d-inline-flex align-items-center gap-2 gf-border-2 rounded-pill px-4 py-2 fw-bold text-black mb-4" style="background: #94FFB4; box-shadow: 3px 3px 0 #000; transform: rotate(-1deg);">
              ...
              <div class="d-flex flex-column flex-sm-row gap-3">
                <a href="#sale-games" class="btn gf-border gf-shadow gf-press fw-bold fs-5 px-4 py-3 rounded-3 d-inline-flex align-items-center justify-content-center gap-2" style="background: var(--gf-green);">
              ...
              <div class="row row-cols-2 g-4">
                <c:forEach var="game" items="${games}" varStatus="status">
                  <c:if test="${status.index lt 4}">
                    ...
                    <c:choose>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'cyberpunk 2077')}"><c:set var="tagText" value="HOT" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'elden ring')}"><c:set var="tagText" value="GOTY" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'baldur')}"><c:set var="tagText" value="RPG" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'black myth') || fn:contains(fn:toLowerCase(game.title), 'wukong')}"><c:set var="tagText" value="New" /></c:when>
                    </c:choose>

                    <c:set var="devName" value="${primaryCategory}" />
                    <c:choose>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'cyberpunk 2077')}"><c:set var="devName" value="CD Projekt Red" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'elden ring')}"><c:set var="devName" value="FromSoftware" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'baldur')}"><c:set var="devName" value="Larian Studios" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'black myth') || fn:contains(fn:toLowerCase(game.title), 'wukong')}"><c:set var="devName" value="Game Science" /></c:when>
                    </c:choose>

                    <c:set var="ratingStars" value="★★★★★ 4.7" />
                    <c:choose>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'cyberpunk 2077')}"><c:set var="ratingStars" value="★★★★★ 4.8" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'elden ring')}"><c:set var="ratingStars" value="★★★★★ 4.9" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'baldur')}"><c:set var="ratingStars" value="★★★★★ 4.9" /></c:when>
                      <c:when test="${fn:contains(fn:toLowerCase(game.title), 'black myth') || fn:contains(fn:toLowerCase(game.title), 'wukong')}"><c:set var="ratingStars" value="★★★★★ 4.8" /></c:when>
                    </c:choose>

                    <c:set var="isDiscounted" value="false" />
                    <c:if test="${fn:contains(fn:toLowerCase(game.title), 'cyberpunk') || fn:contains(fn:toLowerCase(game.title), 'elden') || fn:contains(fn:toLowerCase(game.title), 'baldur')}">
                    ...
                    <div class="col">
                      <article class="gf-game-card gf-press game-item" data-id="${game.id}" data-title="${fn:escapeXml(fn:toLowerCase(game.title))}" data-category="${fn:escapeXml(fn:toLowerCase(primaryCategory))}">
                        <div class="gf-game-banner">
                          ...
                          <button type="button" onclick="toggleFavorite(event, '${game.id}')" class="gf-favorite-btn favorite-button" data-game-id="${game.id}" title="Yêu thích">
                        ...
                        <div class="p-3 p-sm-4 d-flex flex-column flex-grow-1">
                          <div class="d-flex align-items-start justify-content-between gap-2 mb-3">
                            <div class="flex-grow-1" style="min-width: 0;">
                              <h3 class="fs-6 fw-black fw-bold text-truncate mb-1" title="${fn:escapeXml(game.title)}">${game.title}</h3>
                              <div class="small fw-semibold text-secondary gf-muted text-truncate">${devName}</div>
                            </div>
                            <c:set var="gameIdStr" value=",${game.id}," />
                            <c:choose>
                              <c:when test="${not empty ownedGameIds && fn:contains(ownedStr, gameIdStr)}">
                                <button type="button" class="btn btn-sm gf-border-2 fw-bold text-dark px-2 bg-light text-secondary" style="border-radius: 8px; font-size:11px; cursor:default; height:38px;" disabled title="Đã có trong thư viện">
                                  <i data-lucide="library" width="14" height="14" class="me-1"></i>Đã sở hữu
                                </button>
                              </c:when>
                              <c:otherwise>
                                <button type="button" onclick="quickAddToCart(event, '${game.id}')" class="gf-cart-btn cart-button" data-game-id="${game.id}" title="Thêm vào giỏ">
                            ...
                            <c:choose>
                              <c:when test="${isDiscounted}">
                                <span class="gf-price-discounted text-success fw-black fw-bold me-2">
                                  <fmt:formatNumber value="${game.price}" type="number" maxFractionDigits="0" /><u>đ</u>
                                </span>
                                <span class="gf-price-original text-muted text-decoration-line-through small">
                                  <fmt:formatNumber value="${game.price * 1.4}" type="number" maxFractionDigits="0" /><u>đ</u>
                                </span>
                              </c:when>
                              <c:otherwise>
          ...
          <div class="row align-items-start g-5">
            <div class="col-lg-6">
              ...
              <p class="gf-hero-text mb-5">
                Game Hot chuyển từng game bằng carousel ngang. Sale Hot vẫn giữ grid cũ 8 game/trang nhưng dùng hiệu ứng trượt nhẹ, không nhấp nháy.
              </p>

              <div class="d-flex flex-column flex-sm-row gap-3 mb-5">
                <a href="#hot-games" class="btn gf-border gf-shadow gf-press fw-black fw-bold fs-5 px-4 py-3 rounded-3 d-flex align-items-center justify-content-center gap-2" style="background:var(--gf-green);">
                  Xem Game Hot <i data-lucide="arrow-right" width="20" height="20"></i>
                </a>
                <a href="#sale-games" class="btn gf-border gf-shadow gf-press fw-black fw-bold fs-5 px-4 py-3 rounded-3 d-flex align-items-center justify-content-center gap-2" style="background:var(--gf-blue);">
              ...
              <div class="gf-stat">
                <div class="row row-cols-2 row-cols-sm-4 g-4">
                  <div class="col"><div class="display-6 fw-black fw-bold">10K+</div><div class="fw-bold text-secondary gf-muted">Tựa Game</div></div>
                  <div class="col"><div class="display-6 fw-black fw-bold">2M+</div><div class="fw-bold text-secondary gf-muted">Game Thủ</div></div>
                  <div class="col"><div class="display-6 fw-black fw-bold">500+</div><div class="fw-bold text-secondary gf-muted">Nhà Phát Hành</div></div>
                  <div class="col"><div class="display-6 fw-black fw-bold">4.9★</div><div class="fw-bold text-secondary gf-muted">Đánh Giá</div></div>
                </div>
              </div>
            </div>
  ...
  <section class="gf-section-soft py-5">
    <div class="container-xl py-4">
      <div class="text-center mb-5">
        <span class="d-inline-block gf-border-2 gf-shadow-sm rounded-pill px-3 py-1 fw-black fw-bold text-uppercase small" style="background:var(--gf-yellow);color:#000;">Danh Mục</span>
        <h2 class="display-5 fw-black fw-bold mt-3">Khám Phá Theo <span class="gf-green">Thể Loại</span></h2>
      </div>

      <div class="row row-cols-2 row-cols-sm-3 row-cols-lg-6 g-4">
        <div class="col"><div class="gf-category-card gf-press"><div class="gf-category-icon mb-3" style="background:var(--gf-pink);"><i data-lucide="swords" width="28" height="28"></i></div><h3 class="fs-6 fw-black fw-bold">Hành Động</h3><p class="small fw-semibold text-secondary gf-muted mb-0">Action RPG</p></div></div>
        <div class="col"><div class="gf-category-card gf-press"><div class="gf-category-icon mb-3" style="background:var(--gf-blue);"><i data-lucide="brain" width="28" height="28"></i></div><h3 class="fs-6 fw-black fw-bold">Chiến Thuật</h3><p class="small fw-semibold text-secondary gf-muted mb-0">Strategy</p></div></div>
        <div class="col"><div class="gf-category-card gf-press"><div class="gf-category-icon mb-3" style="background:var(--gf-lavender);"><i data-lucide="users" width="28" height="28"></i></div><h3 class="fs-6 fw-black fw-bold">Nhập Vai</h3><p class="small fw-semibold text-secondary gf-muted mb-0">RPG</p></div></div>
        <div class="col"><div class="gf-category-card gf-press"><div class="gf-category-icon mb-3" style="background:#94FFB4;"><i data-lucide="car" width="28" height="28"></i></div><h3 class="fs-6 fw-black fw-bold">Đua Xe</h3><p class="small fw-semibold text-secondary gf-muted mb-0">Racing</p></div></div>
        <div class="col"><div class="gf-category-card gf-press"><div class="gf-category-icon mb-3" style="background:var(--gf-yellow);"><i data-lucide="package" width="28" height="28"></i></div><h3 class="fs-6 fw-black fw-bold">Software</h3><p class="small fw-semibold text-secondary gf-muted mb-0">Tools</p></div></div>
        <div class="col"><div class="gf-category-card gf-press"><div class="gf-category-icon mb-3" style="background:#fbcfe8;"><i data-lucide="heart" width="28" height="28"></i></div><h3 class="fs-6 fw-black fw-bold">Giải Trí</h3><p class="small fw-semibold text-secondary gf-muted mb-0">Casual</p></div></div>
      </div>
    </div>
  </section>
    ...
    <div class="container-xl py-4">
      <div class="d-flex flex-column flex-lg-row align-items-start align-items-lg-end justify-content-between gap-4 mb-5">
        ...
        <div class="d-flex flex-wrap align-items-center gap-3">
          <button id="toggleFavoriteOnlyHot" class="btn gf-border gf-shadow-sm gf-press fw-black fw-bold rounded-3 d-flex align-items-center gap-2" type="button">
          ...
          <div class="gf-border gf-shadow-sm rounded-3 px-3 py-2 fw-black fw-bold bg-white text-dark">
            <span id="hotCurrentPage">01</span><span class="text-secondary"> / </span><span id="hotTotalPages">01</span>
          </div>
        </div>
      </div>

      <div class="gf-slider-shell gf-slider-padding">
        <button id="hotPrev" class="gf-side-nav-btn gf-side-left" type="button" aria-label="Game hot trước"><i data-lucide="chevron-left" width="28" height="28"></i></button>
                  ...
                  <article class="gf-game-card gf-press">
                    ...
                    <div class="p-4 d-flex flex-column flex-grow-1">
                      <div class="d-flex align-items-start justify-content-between gap-3 mb-3">
                        <div class="flex-grow-1" style="min-width: 0;">
                          <h3 class="fs-5 fw-black fw-bold text-truncate mb-1" title="${fn:escapeXml(game.title)}">${game.title}</h3>
                          <p class="small fw-semibold text-secondary gf-muted text-truncate mb-0">${devName}</p>
                        </div>
                        <c:set var="gameIdStr" value=",${game.id}," />
                        <c:choose>
                          <c:when test="${not empty ownedGameIds && fn:contains(ownedStr, gameIdStr)}">
        ...
        <button id="hotNext" class="gf-side-nav-btn gf-side-right" type="button" aria-label="Game hot sau"><i data-lucide="chevron-right" width="28" height="28"></i></button>
      </div>
  ...
  <section id="sale-games" class="gf-sale-section py-5">
    <div class="container-xl py-4">
      <div class="d-flex flex-column flex-lg-row align-items-start align-items-lg-end justify-content-between gap-4 mb-5">
        <div>
          <div class="d-inline-flex align-items-center gap-2 gf-border gf-shadow-sm rounded-pill px-4 py-2 fw-black fw-bold text-white mb-3" style="background:#f87171;">
            <i data-lucide="zap" width="20" height="20"></i> FLASH SALE ĐANG DIỄN RA!
          </div>
          <h2 class="display-5 fw-black fw-bold">Sale Hot <span class="text-danger" style="text-shadow:2px 2px 0 #000;">Nhiều Game</span></h2>
          <p class="fw-semibold text-secondary gf-muted mb-0">Grid động từ dữ liệu game, <strong>${fn:length(games)}</strong> game, 8 game/trang, hiệu ứng trượt mượt không nhấp nháy.</p>
        </div>

        <div class="d-flex flex-wrap align-items-center gap-3">
          <button id="toggleFavoriteOnlySale" class="btn gf-border gf-shadow-sm gf-press fw-black fw-bold rounded-3 d-flex align-items-center gap-2" type="button">
            <i data-lucide="heart" width="16" height="16"></i> Chỉ yêu thích
          </button>
          <div class="gf-border gf-shadow-sm rounded-3 px-3 py-2 fw-black fw-bold bg-white text-dark">
            <span id="saleCurrentPage">01</span><span class="text-secondary"> / </span><span id="saleTotalPages">01</span>
          </div>
        </div>
      </div>
      ...
      <div class="gf-slider-shell gf-slider-padding">
        ...
        <div class="gf-sale-grid-shell">
          <div id="saleGrid" class="gf-sale-grid row row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xl-4 g-4">
                      ...
                      <c:choose>
                        <c:when test="${isDiscounted}">
                          <span class="gf-price-discounted">
                            <fmt:formatNumber value="${game.price * (100 - salePercent) / 100}" type="number" maxFractionDigits="0" /><u>đ</u>
                          </span>
            ...
            <c:if test="${empty games}">
              <div class="col-12">
                <div class="bg-white gf-border rounded-4 p-5 text-center gf-shadow">
                  <div class="mx-auto mb-4 gf-border rounded-4 gf-shadow-sm d-grid place-items-center" style="width:64px;height:64px;background:var(--gf-pink);color:#000;place-items:center;">
        ...
        <button id="saleNext" class="gf-side-nav-btn gf-side-right text-white" style="background:#f87171;" type="button" aria-label="Sale sau"><i data-lucide="chevron-right" width="28" height="28"></i></button>
      </div>
  ...
  <section id="community" class="py-5">
    <div class="container-xl py-4">
      <div class="text-center mb-5">
        <span class="d-inline-block gf-border-2 gf-shadow-sm rounded-pill px-3 py-1 fw-black fw-bold text-uppercase small" style="background:var(--gf-lavender);color:#000;">Đánh Giá</span>
        <h2 class="display-5 fw-black fw-bold mt-3">Game Thủ <span class="gf-green">Nói Gì?</span></h2>
      </div>

      <div class="row row-cols-1 row-cols-md-3 g-4">
        <div class="col"><div class="gf-game-card p-4"><div class="text-warning mb-3">★★★★★</div><p class="fw-semibold text-secondary gf-muted mb-0">"Sale vẫn là grid cũ nên dễ xem, nhưng chuyển trang mượt hơn nhiều."</p></div></div>
        <div class="col"><div class="gf-game-card p-4"><div class="text-warning mb-3">★★★★★</div><p class="fw-semibold text-secondary gf-muted mb-0">"Game Hot trượt từng card không còn chớp tắt."</p></div></div>
        <div class="col"><div class="gf-game-card p-4"><div class="text-warning mb-3">★★★★☆</div><p class="fw-semibold text-secondary gf-muted mb-0">"Bootstrap 5 nhưng vẫn giữ được chất neo-brutalism."</p></div></div>
      </div>
    </div>
  </section>
<section id="newsletter" class="py-5" style="background: var(--gf-green); border-top:3px solid #000; border-bottom:3px solid #000;">
  <div class="container-xl py-4">
    <div class="mx-auto bg-white gf-border rounded-4 gf-shadow p-4 p-md-5 text-center" style="max-width: 760px;">
      <div class="mx-auto mb-4 gf-border rounded-4 gf-shadow-sm d-grid" style="width:72px;height:72px;place-items:center;background:var(--gf-blue);color:#000;">
      ...
      <form action="${pageContext.request.contextPath}/newsletter/subscribe" method="post" class="row g-3 justify-content-center">
        ...
        <div class="col-12 col-md-auto">
          <button type="submit" class="btn bg-dark text-white gf-border gf-shadow-sm gf-press rounded-3 fw-black fw-bold px-4 py-3 w-100">
            Đăng Ký
          </button>
        </div>
      </form>
